...
# ==========================================
# 1. АНАЛИЗАТОР ФИЗИКИ (БАЗА 01.03 + ARESTI SPEED)
# ==========================================
def does_figure_change_axis(aresti_list):
    ...
    for code in aresti_list:
        ...
        if len(parts) == 4:
            ...
            row = int(parts[2])
            col = int(parts[3])
            # Виражи (Fam 2): 90° и 270° меняют ось
            if family == 2 and sub in [1, 3]: changes = not changes 
            # Вращения (Fam 9): нечетные (1/4, 3/4) на вертикалях и в штопорах меняют ось
...
def analyze_figure(f_data):
    ...
    # --- ОРИГИНАЛЬНЫЙ ПАРСЕР ПОЛОЖЕНИЙ 01.03 (Не тронут) ---
    ...
    if family == 6: base_flip = True # Колокола всегда меняют положение
    ...
    # --- НОВАЯ СОВЕРШЕННАЯ МАТРИЦА СКОРОСТЕЙ ПО АРЕСТИ (Защита от алиасов) ---
    ...
    if family == 1:
        ...
        if sub in [1, 2]:
            if row in [1, 2]: starts_up = True; exits_down = True
            if row in [3, 4]: starts_down = True; exits_up = True
        elif sub in [3, 4]:
            if row in [1, 2, 5]: starts_up = True; exits_down = True
            if row in [3, 4, 6]: starts_down = True; exits_up = True
    elif family == 8:
        if sub in [1, 2, 3, 4, 13, 14, 5, 6]: 
...
def build_tournament_sequence(num_hard, num_link, max_k_total, link_threshold):
    ...
    for i in range(length):
        ...
        # === ВОССТАНОВЛЕН СТРОЖАЙШИЙ ФИЛЬТР СКОРОСТЕЙ ИЗ 01.03 ===
        speed_filtered = []
        for f in valid_figs:
            req = f["req_speed"]
            if current_speed == 'HS' and f.get("has_flick"): continue
            
            # Та самая идеальная стыковка:
...
st.title("🏆 Unlimited Pro (The Perfect 01.03 Upgrade)")
st.write("Сборка от 01.03. Скорости и положения теперь вычисляются математически по кодам Арести, что гарантирует 100% распознавание любых судейских алиасов.")
